...
<link rel="stylesheet" href="<%=basePath %>css/jqueryeasyresponsivetabs/easy-responsive-tabs.css" type="text/css"></link>
			...
			<div style="float:left;width:50%">
				<table class="search-tab1" style="margin:auto;background-color: #2ECCFA;text-align: center">
					...
					<tr>
						...
						<td width="80px">&#8451;</td>
						<%
							if(systemno.equals("2")){// 面源系统才使用一下图像
						%>
						<%--<td width="50%">流量</td>--%>
...
<div id="parentHorizontalTab">
	<ul class="resp-tabs-list hor_1">
		...
		<%
			if(systemno.equals("0")){// 无锡老系统使用
		%>
		<li><a href="#tabs-10">COD</a></li>
		<li><a href="#tabs-11">氨氮</a></li>
		<li><a href="#tabs-13">总磷</a></li>
		<li><a href="#tabs-99">气压</a></li>
		<%
			}
		%>

		<%
			if(systemno.equals("1")){// 江都系统
		%>
	...
	<div class="resp-tabs-container hor_1">
		...
		%>
		<div id="tabs-10">
			<div id="chart10" style="height:300px;width:1000px">
			</div>
		</div>
		<div id="tabs-11">
			<div id="chart11" style="height:300px;width:1000px">
			</div>
		</div>
		<div id="tabs-13">
			<div id="chart13" style="height:300px;width:1000px">
			</div>
		</div>
		<div id="tabs-99">
			<div id="chart99" style="height:300px;width:1000px">
			</div>
		</div>
		<%
			}
		...
		<div id="tabs-12">
			<div id="chart12" style="height:300px;width:1000px">
			</div>
		</div>
		<div id="tabs-13">
			<div id="chart13" style="height:300px;width:1000px">
		...
		<div id="tabs-14">
			<div id="chart14" style="height:300px;width:1000px">
			</div>
		</div>
		<%
			}
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if($("#systemno").val() == 1){//无锡使用
			chart6();
		}

		if($("#systemno").val() == 2){//面源使用
			chart10();
			chart11();
			chart12();
			chart13();
		...
		if($("#systemno").val() == 0){//无锡使用
			$("#showTrueCurve").change(function(){
				//如果打钩表示要展示真实数据
				if( $("#showTrueCurve").is(':checked') == true){
					getTrueCurveInfo();
				}else{
					getFakeCurveInfo();
				}
...
//			}
				...
				$("#picBackground").css("background-image", "url(<%=basePath %>images/sewageMonitorBG/"+controlMethod+".jpg)");
			}
					...
					for(var j = 0; j < data.length; j++){
						if(i == (data[j].hourname + 1)){ //时间是否相等
							...
							if($("#systemno").val() == 2){//面源使用
								...
								var de10 = (data[j].detection10/hourcount).toFixed(2);
								var de11 = (data[j].detection11/hourcount).toFixed(2);
								var de12 = (data[j].detection12/hourcount).toFixed(2);
								var de13 = (data[j].detection13/hourcount).toFixed(2);
								var de14 = (data[j].detection14/hourcount).toFixed(2);
