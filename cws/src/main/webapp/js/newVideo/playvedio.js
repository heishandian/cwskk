...
function fetch_resource(){
	...
	if(rv.rv == P_Error.SUCCESS){
		for(var i =0;i <rv.response.length;i++){
			...
			if(pu.modelType == "ENC" || pu.modelType == "WENC"){
				...
				parentNodes.push({id:pu._HANDLE,text:pu.name,attributes:{pu:pu}});
			}
		}
		if(nodes.length > 0){
	...
	var puid = $("#vediopuid").val();
	var name =  $("#vedioname").val();
	for(var j = 0;j < parentNodes.length; j ++){
		if(parentNodes[j].attributes.pu.puid == puid){
			var rv = P_LY. ForkResource(connectId,P_LY.Enum.ForkResourceLevel.nppForkPUResourceInfo, 0,120, "",{puid:parentNodes[j].attributes.pu.puid,_HANDLE:parentNodes[j].id});
			for(var k = 0; k<rv.response.length;k++){
				var pure = rv.response[k];
				
				// 这里只列出视频输入资源
				if(pure.type == P_LY.Enum.PuResourceType.VideoIn){
					if(pure.name == name){
						//播放视频
						play(parentNodes[j].attributes.pu,pure);
					}
				}
			}
		}
...
function stopmove(){
	...
	var mode = P_LY.Enum.PTZDirection.stopturn;
