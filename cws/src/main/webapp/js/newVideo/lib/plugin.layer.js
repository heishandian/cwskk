    ...
    Init : function (initParam)
    {
        try 
        {
            ...
            if(initParam.type == "external"){
    			...
    			if(initParam.type == "embed") P_LY.Plug.type = "e";
    ...
    */
	...
	GetDomainRoad : function (connectId)
	{
		...
		if (operator.rv == P_Error.SUCCESS)
		{
			...
			{
				subdm.push(P_Utils.UTF8toUnicode(operator.response[i]));	
			}
    ...
    **/ 
	Download :
	{
		...
		*/
		...
		StartCloudFileDownload : function (connectId, szId, fileAllPath, localSaveAsFile)
		{
			return P_LY.Download.__Sdk_DLControl
			(
			 	"cloud",
		...
		*/
		...
		params:
			...
			- localSaveAsFile(string) * 下载保存全路径，如C:/vod.avi，上层调用时确保正确的存储路径
		*/
		StartCEFSVodFileDownload : function (connectId, puid, sgIndex, remoteFile, localSaveAsFile)
		{
			return P_LY.Download.__Sdk_DLControl
			(
			 	"dlcefsvod",
				...
				switch (action)
				{
					case "cloud": // - 下载平台录像或图片
						...
						{		
							...
							if (_connStruct.status == P_LY.Enum.ConnectionStatus.Connected)
							{
								operator = P_IF.DownloadCloudRecord(_connStruct.nc,_connStruct.session,options.szId,options.fileAllPath,options.localSaveAsFile);
	...
	CommonRequest : function(connectId, options)
	{
		...
		{	
			...
			if (_connStruct.status == P_LY.Enum.ConnectionStatus.Connected)
			{
				...
				if(requestXML.search('C_CSS_QueryStorageFiles') > 0){
