...
var P_IF = 
{
				...
				if (!_method || _argslen < (_minArgsCount + 1)) 
				{
					...
					else
					{
						...
						switch(_objType)
						{
							case P_IF.Enum.ObjectType.NC :
								try{
									if(_method == "EnableDrawRect") console.log(operator);
	...
	*/
	...
	SG_DownLoadFile : function (nc, sgHandle, remoteFile, localSaveAsPath)
	{
		return P_IF.Common.GetNCResponse("DownloadRecord", nc, sgHandle,  remoteFile, localSaveAsPath);
	},
	// - 下载CEFS文件系统的图片
	SG_CEFSDownLoadSnapshot : function (nc, sgHandle, snapshotTime, noInSecond, ivIndex, localSaveAsPath)
	{
		return P_IF.Common.GetNCResponse("SG_CEFSDownLoadSnapshot", nc, sgHandle, snapshotTime, noInSecond, ivIndex, localSaveAsPath);
	},
	...
	DownloadCloudRecord:function(nc,handle,szId,fileAllPath,localSaveAsPath){
		
		return P_IF.Common.GetNCResponse("CSSDownloadRecord", nc,handle, szId, fileAllPath, localSaveAsPath); 
	},
	...
	*/
    ...
    Enum : 
    {
		...
		PTZDirection :
		{
			...
			"runtour" : "C_PTZ_RunTour", // - 启动巡航
			"stoptour" : "C_PTZ_StopTour", // - 停止巡航
			"startautoscan" : "C_PTZ_StartAutoScan", // - 启动自动扫描
			"stopautoscan" : "C_PTZ_StopAutoScan", // - 启动自动扫描
			"setbeginpos" : "C_PTZ_SetAutoScanBeginPos", // - 设置自动扫描起点
			"setendpos" : "C_PTZ_SetAutoScanEndPos" // - 设置自动扫描终点
